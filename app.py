...
if (len(st.session_state.messages) <= 8 and
    ("token" in st.session_state or st.session_state.token == "")) or (
        validate_openai_api_key(st.session_state.token)
        and "token" in st.session_state):
    if prompt := st.chat_input("Wassup"):
        ...
        response, docs = trigger_event(
            st.session_state.token if "token" in st.session_state
            and validate_openai_api_key(st.session_state.token) else
            env["OPENAI_API_KEY"])
